...
@Entity @Table(name="b2b_product_catalog") public class ProductCatalog extends BaseEntity {
    ...
    protected ProductCatalog(){} /**
                                  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
                                  */
public ProductCatalog(String code,String name,BusinessAccount businessAccount,Status status,int oee){this.code=code;this.name=name;this.businessAccount=businessAccount;this.status=status;this.oee=oee;this.lastHeartbeat=LocalDateTime.now();}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
